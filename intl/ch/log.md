...
**思考：** 前端早读主要介绍了WebAssembly，具体使用还有很远，简单了解。
...
**思考：** 前端早读主要介绍了页面链接的打开方式，即在当前页面打开还是新页面打开。
...
**思考：** 前端早读主要介绍了JS和WebAssembly相互调用。
...
**思考：** 前端早读主要介绍了谷歌photo的实现，主要是点是根据所有图片计算出大体的框架结构，并按列别分块。然后根据算法计算出每行显示图片的最佳数量，保持最佳的图片比例。当用户滑动窗口，按分类显示/隐藏图片，实现懒加载。最后当用户点击放大，先放大缩略图，同时加载原图。