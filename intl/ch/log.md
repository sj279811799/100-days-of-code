...
**今天的进展：** 阅读了早读文章，学习了前端性能优化0.5。

**思考：** 前端早读主要介绍了对css进行重命名来避免相互影响，主要是在css名称后加一串随机码。前端性能优化0.5主要是对是代码的打包时间和大小的优化。主要是避免node_modules的babel转义，将第三方依赖单独打包一个文件缓存，只有改变时才重新编译。开启多进程处理。体积压缩主要是删除冗余代码和按需加载。